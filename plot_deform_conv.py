...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        offset11_output, offset12_output, offset21_output, offset22_output = sess.run(
            [offset11, offset12, offset21, offset22],
            feed_dict={input_placeholder: input / 255.,
                       is_training_placeholder: False})
        # The moving average of batch normalization has to be frozen
        frames = []
        for i in range(total_frames):
            offset_11_plot = plot_offset(img=input[i, 0, :, :], offset=offset11_output[i, ...], kernel_size=3, stride=1,
                                         padding="SAME", thres=0.1, text="offset_11, thres=0.1")
            offset_12_plot = plot_offset(img=input[i, 0, :, :], offset=offset12_output[i, ...], kernel_size=3, stride=2,
                                         padding="SAME", thres=0.5, text="offset_12, thres=0.5")
